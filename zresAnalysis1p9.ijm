...
// Print the results as a tab-delimited list to paste into the ACR worksheet
// Shorten the two mean measurements to one decimal with d2s command (double to string)
print("\\Clear"); // clear the current log window
for (j = 0; j < 2*nn+1; j++) {
	print(results[j*10]+" \t"+results[j*10+1]+" \t"+results[j*10+2]+" \t"+results[j*10+3]+
...
// Create a diaglog box to indicate the results were copied to the clipboard
// Clicking OK will copy the montage to the clipboard
...
Dialog.addMessage("Results copied to clipboard");
Dialog.addMessage("Click OK to copy montage");
Dialog.addCheckbox("Close all after copying", false)
Dialog.show();
closeCheck = Dialog.getCheckbox();
